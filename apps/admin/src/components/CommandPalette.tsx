...
export function CommandPalette() {
  ...
  const commands = useMemo<Command[]>(() => {
    ...
    // Gated on the settings capability: editors never see the wizard.
    const setupCommands: Command[] = window.flexaSeoAeo?.canManageSettings
      ? [
          {
            id: "run:setup",
            label: __("Run Setup Assistant"),
            hint: __("Re-run detection and recommendations"),
            icon: Sparkles,
            keywords: "setup assistant onboarding wizard detect recommend",
            run: () => {
              openSetup();
              setOpen(false);
            },
  ...
  }, [openSection, setView, openSetup, setOpen]);
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    i === index ? "fsa:bg-brand-50" : "fsa:bg-transparent",
                  )}
                >
                  <span
                    className={cn(
